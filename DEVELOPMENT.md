...
Every host builds findings through one evaluator, `createEvaluator()` in `packages/analyzer/src/evaluator.ts` (hot paths import it as `@omnodex/analyzer/evaluator`). It classifies each rule by the state it needs: `event` rules judge one call, `session` rules need earlier events of the session (first-seen, rate and `sequence` conditions). A host runs the classes it can afford: `hook` runs event rules only, `proxy` and `batch` run everything. Deduplication is on rule plus `tool_call_id` and is seeded from every `risk.detected` the evaluator sees, so hosts sharing a log never repeat each other. A `sequence` condition matches when an earlier event in the session satisfied its `prior` conditions; such findings list every event in the pattern in `related_event_ids`. `node scripts/time-evaluator.mjs` measures what the evaluator costs a fresh hook process. The MCP proxy runs an evaluator with host `proxy` over every event it records (`packages/mcp-proxy/src/evaluation.ts`), queued off the response path and loaded on the first tool call. A routed call seen by both a hook and the proxy can raise the same finding twice; the correlation pass gives both findings the pair's `correlation_id`, and `findingKey()` in `@omnodex/projection` counts them once. The hook shims judge each `tool.invoked` they write with the event rules through `judgeCaptured()` (`packages/analyzer/src/capture.ts`, imported as `@omnodex/analyzer/capture`), which loads the evaluator only for a tool call, caps load plus evaluation at `OMNODEX_CAPTURE_DETECT_TIMEOUT_MS` (default 1000), and returns no findings on a timeout or error; the background pass judges those later. Findings are appended after the event and pushed with it in one `pushEventsToCloud` call. `CAPTURE_HOSTS` records which interceptors evaluate at capture, and the dashboard's live loop passes `classesLeftAfterCapture()` to `evaluate()` so it does not race them to write the same finding. The shims' tests load `packages/analyzer/test/helpers/capture-hooks.mjs` with `--import` to make the evaluator throw or hang and to record pushes.

Rules come in two tiers. Community rules ship in this package and are always active. Advanced rules arrive as one sealed bundle per version, the same bytes for every entitled customer, opened on the machine that evaluates (`packages/analyzer/src/bundle.ts`, design in `planning/architecture/PAID_RULE_DELIVERY.md`). `HOST_TIERS` says which hosts run them: the proxy and the batch pass do, a hook shim never does, because it is a process per tool call. `registryForHost(host, home)` is what every host calls; it opens the installation's bundle only when that host may run one, and returns the community set whenever the bundle is missing, unsigned, altered, expired or unopenable, so a bad bundle can never stop detection. `omnodex status` prints which rules are loaded and, when none are, why. For a Pro or Enterprise install, the background pass keeps the bundle current before it detects (`refreshRuleBundle()` in `packages/sync-encryptor/src/rules-refresh.ts`): it fetches `GET /api/v1/rules/update?since=<installed>` into `OMNODEX_HOME/rules/current.bundle.json`, and the key that opens it comes separately, from the licence check, and is cached with the licence in `license-cache.json`. The loader takes the key in that order: one passed in, `OMNODEX_RULES_KEY`, then the licence cache, and a cached key counts only for exactly the installed channel and version. An install whose cached licence is not Pro or Enterprise makes no request for any of this; `OMNODEX_RULES_REFRESH=0` turns the refresh off. To work on advanced rules before any of the cloud side exists, sign a bundle locally and point `OMNODEX_RULES_PUBLIC_KEY` and `OMNODEX_RULES_KEY` at it.

Advanced rules themselves are not in this repository, because it is public and they are the paid asset. Two scripts here work on a pack wherever it lives: `node scripts/seal-bundle.mjs <pack.json> --install <home>` seals and signs one into a bundle and prints the two variables a client needs to open it, and `node scripts/check-rules.mjs <pack> --cases <cases.json>` runs written cases while `--log <event-log root>` replays real activity to see what a pack would fire on before it ships. A case is a session of calls plus the rule ids that must and must not fire on the last one.
